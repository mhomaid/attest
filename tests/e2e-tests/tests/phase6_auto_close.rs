...
async fn phase6_response_always_includes_case_state_and_shadow_check() {
    ...
    let resp = post_triage(&client, &url, json!({
        "severity_score": 0.3,
    ...
    assert!(resp.get("case_state").is_some(), "missing case_state: {resp}");
    assert!(resp.get("shadow_check").is_some(), "missing shadow_check: {resp}");

    let case_state = resp["case_state"].as_str().expect("case_state must be a string");
    assert!(
    ...
    assert!(sc["allowed"].is_boolean(), "shadow_check.allowed must be bool");
    assert!(sc["reason"].is_string(), "shadow_check.reason must be string");
    assert!(sc["policies_evaluated"].is_array(), "shadow_check.policies_evaluated must be array");
}
...
async fn phase6_protected_principal_never_auto_closes() {
    ...
    let do_not_touch = std::env::var("DO_NOT_TOUCH_LIST")
        .unwrap_or_else(|_| "ceo@corp.com".into());
    let protected = do_not_touch.split(',').next().unwrap_or("ceo@corp.com").trim().to_string();
