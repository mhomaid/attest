...
impl CalibrationClient {
    pub fn from_env() -> Self {
        let base_url =
            std::env::var("CALIBRATION_URL").unwrap_or_else(|_| "http://localhost:5001".into());
        Self {
            base_url,
    ...
    pub async fn calibrate(
        ...
        match self
            .try_calibrate(agent_id, case_class, raw_score, path)
            .await
        {
            Ok(score) => score,
            Err(e) => {
    ...
    async fn try_calibrate(
        ...
        let resp: CalibrationResponse = self
            .client
            .post(&url)
            .json(&CalibrationRequest {
                agent_id,
                case_class,
                raw_score,
                path,
